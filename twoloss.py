...
for epoch in range(epochs):
    ...
    for i,batch in enumerate(train_loader):
        ...
        Lambda = 0.5#Lambda 是一个超参数，控制 对比损失 和 原始损失 之间的加权系数。它的值为 0.5，意味着 原始损失/扰动损失 和 对比损失 在总损失中的贡献是相等的。
        total_batch_loss = (1 - Lambda) * original_loss + mlp_similarity_loss * Lambda#这一部分计算的是 原始损失（original_loss）和 扰动损失（perturbed_loss）的加权平均。
